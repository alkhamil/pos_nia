...
class User_model extends CI_Model {
    ...
    public $table = 'm_user';
    public $primary_key = 'm_user.id';
    public $order_by = 'm_user.id';
    public $order_type = 'ASC';
    public $search_field = 'm_user.username';
    public $column_order = ['m_user.username']; //set column field database for datatable orderable
    public $column_search = ['m_user.username']; //set column field database for datatable searchable 
    ...
    function lists($select = '*', $where = null, $limit = 10 ,$offset = 0)
    {
        $this->db->select($select)
                 ->join('m_user_type', 'm_user_type.id = m_user.user_type_id')
                 ->limit($limit,$offset);
