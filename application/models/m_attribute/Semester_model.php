...
class Semester_model extends CI_Model {
    ...
    public $table = 'm_attribute_semester';
    public $primary_key = 'm_attribute_semester.id';
    public $order_by = 'm_attribute_semester.id';
    public $order_type = 'ASC';
    public $search_field = 'm_attribute_semester.name';
    public $column_order = ['m_attribute_semester.name']; //set column field database for datatable orderable
    public $column_search = ['m_attribute_semester.name', 'm_attribute_type.name']; //set column field database for datatable searchable 
    ...
    function lists($select = '*', $where = null, $limit = 10 ,$offset = 0)
    {
        $this->db->select($select)
                 ->join('m_attribute_type', 'm_attribute_type.id = m_attribute_semester.attribute_type_id')
                 ->limit($limit,$offset);
    ...
    function list_count($where = null, $is_where = false) {
        $this->db->join('m_attribute_type', 'm_attribute_type.id = m_attribute_semester.attribute_type_id');
        if($is_where) {
            if($where) {
                if(isset($where['q']) && $where['q'])
                    $this->db->like("LOWER(".$this->search_field.")", $where['q']);
