...
class Pembayaran extends CI_Controller {
    ...
    public function get_pembayaran_siswa()
    {
        ...
        if ($biaya_lembaga) {
            if ($pembayaran == null) {
                ...
                $data['pembayaran']['checkout'] = $this->get_data_checkout($this->input->get('siswa_id', TRUE), $this->input->get('kelas_id', TRUE));
    ...
    public function get_data_checkout($siswa_id, $kelas_id)
    {
        $where['siswa_id'] = $siswa_id;
        $where['kelas_id'] = $kelas_id;
        $this->Pembayaran_model->table = "t_checkout";
        $this->Pembayaran_model->order_by = "id";
        $checkout = $this->Pembayaran_model->get_all($where, "*");
    ...
    public function get_lembaga_name($id)
    {
        $where['id'] = $id;
        $select = "*";
    ...
    public function get_kelas_name($id)
    {
        $where['id'] = $id;
        $select = "*";
        $this->Pembayaran_model->table = "m_kelas";
        $this->Pembayaran_model->order_by = "id";
        $kelas = $this->Pembayaran_model->get($where, $select);
        $this->Pembayaran_model->table = "t_pembayaran";
        return $kelas->name;
    }
    ...
    public function simpan()
    {
        ...
        $savedata['code'] = $this->check_code($this->get_lembaga_name($savedata['lembaga_id']), $this->get_kelas_name($savedata['kelas_id']));
        ...
        if($this->input->post('id')) { 
            ...
            //create
            $where['tahun_ajaran_id'] = $this->input->post('tahun_ajaran_id', TRUE);
            $where['lembaga_id'] = $this->input->post('lembaga_id', TRUE);
            $where['siswa_id'] = $this->input->post('siswa_id', TRUE);
            $where['kelas_id'] = $this->input->post('kelas_id', TRUE);

            $pembayaran = $this->Pembayaran_model->get_all($where);
            ...
            if ($pembayaran_id) {
                ...
                $this->store_checkout($list_pembayaran, $savedata['siswa_id'], $savedata['kelas_id']);
    ...
    public function store_checkout($pembayaran, $siswa_id, $kelas_id)
    {
        foreach ($pembayaran->komite as $key => $item) {
            if ($item->is_checkout == 1) {
                $savedata['siswa_id'] = $siswa_id;
                $savedata['kelas_id'] = $kelas_id;
                $savedata['biaya_type'] = 'komite';
                $savedata['biaya_type_id'] = $item->id;
    ...
    public function check_code($lembaga, $kelas)
    {
        ...
        $result = 'IN/'.$lembaga.'/'.$kelas.'/'.date('Ym').'/'.str_pad($code + 1, 4, "0", STR_PAD_LEFT);
        return $result;
    }
